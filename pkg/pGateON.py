...
class network(torch.nn.Module):
    def __init__(
        ...
        if self.conv:
            ...
            for layer in range(self.n_conv):
                ...
                self.conv_availabilities += [
                    torch.ones_like(self.FF_convs[-1].weight).to(device)
                ]
                self.conv_availabilities_bias += [
                    torch.ones_like(self.FF_convs[-1].bias).to(device)
                ]
        ...
        for layer in range(self.n_layers):
            ...
            self.availabilities += [torch.ones(o, i).to(device)]
            self.availabilities_bias += [torch.ones(o).to(device)]
            self.FF_layers += [torch.nn.Linear(i, o).to(device)]

        if self.conv:
            self.tmp_conv_availabilities = copy.deepcopy(self.conv_availabilities)
            self.tmp_conv_availabilities_bias = copy.deepcopy(
                self.conv_availabilities_bias
            )
        ...
        self.tmp_availabilities_bias = copy.deepcopy(self.availabilities_bias)
        ...
        if "age" in self.lambda_:
            if self.conv:
                for layer in range(self.n_conv):
                    ...
                    print(
                        "Number of available parameters conv layer {0}: ".format(layer),
                        1 - torch.mean(ages),
                    )
            for layer in range(self.n_layers - 1):
                ages = 1.0 * (self.tmp_availabilities[layer] < 0.05)
                print(
                    "Number of available parameters layer {0}: ".format(layer),
                    1 - torch.mean(ages),
                )
    ...
    def update_learning_rate(self):
        if "age" in self.lambda_:
            if self.conv:
                for layer in range(self.n_conv):
                    self.FF_convs[layer].bias.grad *= self.conv_availabilities_bias[
                        layer
                    ].detach()
                    self.FF_convs[layer].weight.grad *= self.conv_availabilities[
                        layer
                    ].detach()
            for layer in range(self.n_layers - 1):
                self.FF_layers[layer].bias.grad *= self.availabilities_bias[
                    layer
                ].detach()
                self.FF_layers[layer].weight.grad *= self.availabilities[layer].detach()
    ...
    def fixing_ages(self):
        ...
        self.availabilities_bias = copy.deepcopy(self.tmp_availabilities_bias)
        if self.conv:
            self.conv_availabilities = copy.deepcopy(self.tmp_conv_availabilities)
            self.conv_availabilities_bias = copy.deepcopy(
                self.tmp_conv_availabilities_bias
            )
    ...
    def update_ages(self, layer, type="full"):

        with torch.no_grad():
            lr = 0.01
            if type == "full":
                post = torch.pow(
                    self.FF_layers[layer].weight.grad * self.FF_layers[layer].weight, 2
                )
                post /= torch.mean(post)
                Delta = lr * self.tmp_availabilities[layer] * (post - self.epsilon)
                post = torch.pow(
                    self.FF_layers[layer].bias.grad * self.FF_layers[layer].bias, 2
                )
                post /= torch.mean(post)
                Delta_bias = (
                    lr * self.tmp_availabilities_bias[layer] * (post - self.epsilon)
                )

            if type == "activity":
                post = torch.einsum(
                    "bi,bj->ij",
                    torch.pow(self.inputs[layer + 1], 2),
                    torch.pow(self.inputs[layer], 2),
                )
                post /= torch.mean(post)
                Delta = lr * self.tmp_availabilities[layer] * (post - self.epsilon)

                post = torch.mean(torch.pow(self.inputs[layer + 1], 2), dim=0)
                post /= torch.mean(post)
                Delta_bias = (
                    lr * self.tmp_availabilities_bias[layer] * (post - self.epsilon)
                )
            self.tmp_availabilities[layer] = self.dA(
                self.tmp_availabilities[layer], Delta
            )
            self.tmp_availabilities_bias[layer] = self.dA(
                self.tmp_availabilities_bias[layer], Delta_bias
            )

    def update_ages_conv(self, layer):
        lr = 0.01
        post = torch.pow(
            self.FF_convs[layer].weight.grad * self.FF_convs[layer].weight, 2
        )
        post /= torch.mean(post)
        Delta = lr * self.tmp_conv_availabilities[layer] * (post - self.epsilon)
        post = torch.pow(self.FF_convs[layer].bias.grad * self.FF_convs[layer].bias, 2)
        post /= torch.mean(post)
        Delta_bias = (
            lr * self.tmp_conv_availabilities_bias[layer] * (post - self.epsilon)
        )

        self.tmp_conv_availabilities[layer] = self.dA(
            self.tmp_conv_availabilities[layer], Delta
        )
        self.tmp_conv_availabilities_bias[layer] = self.dA(
            self.tmp_conv_availabilities_bias[layer], Delta_bias
        )
