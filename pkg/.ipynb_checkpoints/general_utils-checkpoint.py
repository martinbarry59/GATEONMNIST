...
def get_options(parser):
    ...
    parser.add_argument("--n_layers",type=int, help = "number of hidden layer + 1", default = 5)
    parser.add_argument("--hidden",type=int, help = "number of hidden neurons ", default = 5000)
    parser.add_argument("--epochs",type=int, help = "number of batch training ", default = 2500)
    parser.add_argument("--approach",type=str, help = "pgateon or gateon ", default = 'ngateon')
    parser.add_argument("--fake_context", help = "use a fake context for task n (list, deprecated)", default = None)
    parser.add_argument("--skip_context",type=list, help = "Skip a context during traing (list) ", default = [])
    parser.add_argument("--dic", help = "if you want to reuse previous data", default = None)
    parser.add_argument("--task",type = str.upper, choices = ['PM','RM','SLM'], help = "which meta task to run ", default = 'PM')
    parser.add_argument("--all_steps",type=bool, help = "number of batch training ", default = False)
    ...
    parser.add_argument("--change_timing",type=int, help = "time wihtin the same task ", default = 500)
    parser.add_argument("--epsilon",type=float, help = "number of batch training ", default = 0)
    parser.add_argument("--lambdas",type=str.lower, help = "can be anything in: ['neuron age sparse','no context sparse','context only sparse','age only sparse']; use initials and dash between models", default = 'nas')
    return parser
def plot_average(dic_,keys,skip ,file = None,folder = None,minidx = 0, maxidx = -1, savemodels = False):
    ...
    for key in keys:
        ...
        for lambda_, trials in dic_.items():
            ...
            if lambda_ not in skip:
                ...
                for plots in range(len(all_errors.keys())):
                    average = torch.mean(torch.Tensor(all_errors[plots]),dim=0)#[minidx:maxidx]
                    std = torch.std(torch.Tensor(all_errors[plots]),dim=0)#[minidx:maxidx]
                    if len(figs) <= plots:
                    ...
                    axs[plots].fill_between(torch.arange(average.shape[0]),average-std,average+std,alpha=0.5)
        ...
        for fig_n in range(len(figs)):
            ...
            figs[fig_n].savefig(path+'/results/' +folder+'/'+ plot_key+'.pdf', bbox_inches='tight')
        plt.close('all')
...
def load_data(train, transform, batch_size ,context_size, angle = False, conv = False):
    print(batch_size)
    loader = torch.utils.data.DataLoader(torchvision.datasets.MNIST('data', train=train, download=True,
